...
-(void)viewWillAppear:(BOOL)animated
{
    ...
    if(self.listTypeSegmentedControl.selectedSegmentIndex == kMyListsSegment)
    {
        ...
        UIBarButtonItem *addBarButtonItem = [[UIBarButtonItem alloc] initWithImage:addButtonImage style:UIBarButtonItemStylePlain target:self action:@selector(addNewList:)];
...
-(void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex{
    if (buttonIndex == 1)
    {
        ...
        if([self.listName isEqualToString:@""])
        {
            UIAlertView *errorAlertView = [[UIAlertView alloc] initWithTitle:@"No List Name" message:@"Please type in a name." delegate:nil cancelButtonTitle:@"Ok" otherButtonTitles:nil, nil];
            [errorAlertView show];
